...
module.exports = function(app, cb) {
  ...
  async.series([
    function createAuthor(done) {
      Author.create({name: 'Author 1'}, function(err, people) {
        author = people;
        done(err, people);
      });
    },
    function createReader(done) {
      Reader.create({name: 'Reader 1'}, function(err, people) {
        reader = people;
        done(err, people);
      });
    },
    function createBook(done) {
      Book.create({name: 'Book 1'}, function(err, book) {
